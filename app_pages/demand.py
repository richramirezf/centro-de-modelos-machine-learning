...
def load_demand_model():
    return load_model("demand_model.joblib")


def load_demand_report() -> dict:
    return load_report("demand_report.json")
...
def render_variables_table() -> None:
    ...
    render_features_table_shared(
        rows,
        "Dataset simulado de ventas determinista (20000 registros). Preprocesamiento: OneHotEncoder sobre las categoricas.",
    )
...
with tab_docs:
    ...
    render_model_doc(
        ...
        metrics={
            ...
            "RMSE (test)": f"{report['test']['rmse']:.2f}",
            "MAE (test)": f"{report['test']['mae']:.2f}",
        },
        use_cases=(
